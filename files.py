...
class LogsDef:
    def __init__(self, current_date, filepath, rtb = None, outname = None):
        ...
        self.rtb = rtb if rtb is not None else "mail"

        assert dt.strptime(current_date, '%d.%m.%Y'), "Date format should be dd.mm.yyyy"
...
class GetAttribution:
    ...
    def __init__(self, segments):
        ...
        self.zippath = os.path.join(self.filepath + '\\logs', self.outname + '_' + self.rtb)

    def __str__(self):
        return f"""
            Path in: {self.filepath}
            Wrote out: {self.zippath}
        """
